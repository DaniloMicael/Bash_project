...
function is_date() {
	...
	#para um caminho que os descartará, fazendo com que nada seja mostrado no terminal, tanto no caso de haver erros (se o comando contiver 
	#algum erro, como por exemplo a data entre aspas em '--date="$1"' ser inválida) como no caso de correr tudo bem (ou seja, o output habitual do comando 
	#também não seria mostrado, e também seria redirecionado e descartado)
	...
	case $option in	
		...
		s)
			...
			if is_integer_positive "$OPTARG"; then
				...
				echo "Error: O número introduzido para size não é >= 0."
			fi
		;;
		r)
			option_r=true
		...
		l)
			...
			if is_integer_positive "$OPTARG" && [ "$OPTARG" -gt 0 ]; then
				...
				echo "Error: O número introduzido tem de ser > 0"
			fi
		;;
	esac
done		
...
for dir in "${directories[@]}"; do
	print_output "$dir"
done | sort $sort | if [ "$option_l" = true ]; then head -n "$limit"; else cat; fi 
