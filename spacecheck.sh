...
# Relatório:
# No relatório: forma como organizámos a solução, não devemos colocar muito código, forma como validámos a solução, testesao código
...
function print_output() {
	...
	find "$dir" -type d | while read -r subdir; do
		...
		if [ "$option_n" = true ]; then
			files=$(find "$subdir" -type f -regex "$file_pattern")
		else
			files=$(find "$subdir" -type f)
		fi
		
		if [ -n "$files" ]; then # condição verdadeira se $files não está vazio
		
			while read -r file; do # percorre file a file
...
# Opções possíveis de serem fornecidas como argumento para definir a seleção de ficheiros desejados ao executar o script:
